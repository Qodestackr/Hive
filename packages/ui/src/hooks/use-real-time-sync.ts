...
export function useRealTimeSync<TData extends { id: string }>({
    ...
    const [pendingChanges, setPendingChanges] = React.useState<SyncEvent<TData>[]>([])
    ...
    const simulateIncomingUpdate = () => {
        ...
        if ('status' in updatedItem) {
            const statuses = ['pending', 'completed', 'cancelled']
                ; (updatedItem as any).status = statuses[Math.floor(Math.random() * statuses.length)]
        }
    ...
    const bulkUpdate = (items: TData[]) => {
        items.forEach((item) => queueChange("update", item))
        if (isOnline) {
    ...
    const bulkDelete = (itemIds: string[]) => {
        ...
        itemsToDelete.forEach((item) => queueChange("delete", item))
