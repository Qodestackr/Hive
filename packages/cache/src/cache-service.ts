...
export class CacheService {
	constructor(private redis: Redis | null) {
		if (!redis) {
			logger.warn(
				"CacheService initialized without Redis - operating in fallback mode",
			);
		}
	}
	...
	async get<T>(key: string | CacheKey): Promise<T | null> {
		...
		return withCacheOperation({ operation: "get", key: cacheKey }, async () => {
			try {
	...
	async invalidate(key: string | CacheKey): Promise<void> {
		...
		return withCacheOperation({ operation: "del", key: cacheKey }, async () => {
	...
	async clearAll(): Promise<void> {
		...
		logger.warn(
			"Clearing all cache - this should only be done in development!",
		);

		return withCacheOperation({ operation: "del", key: "*" }, async () => {
	...
	async getStats(): Promise<{
		...
		try {
			...
			const hits = hitRate ? parseInt(hitRate, 10) : 0;
			const totalRequests = hits + (misses ? parseInt(misses, 10) : 0);
			const rate =
				totalRequests > 0 ? ((hits / totalRequests) * 100).toFixed(2) : "0";
