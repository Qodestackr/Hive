 ...
 */
function buildKey(namespace: string, identifier: string, subkey?: string, tags: string[] = []): CacheKey {
...
export const ProductKeys = {
    ...
    inventory: (productId: string, orgId: string): CacheKey =>
        buildKey('products', `inventory:${productId}`, undefined, ['org', 'product', 'inventory']),
...
export const CampaignKeys = {
    ...
    detail: (campaignId: string, orgId: string): CacheKey =>
        buildKey('campaigns', `detail:${campaignId}`, undefined, ['org', 'campaign']),
    ...
    stats: (campaignId: string, orgId: string): CacheKey =>
        buildKey('campaigns', `stats:${campaignId}`, undefined, ['org', 'campaign', 'stats']),
    ...
    promoCode: (code: string, orgId: string): CacheKey =>
        buildKey('campaigns', `promo:${code}`, undefined, ['org', 'campaign', 'promo']),
    ...
    profitCheck: (campaignId: string, orgId: string): CacheKey =>
        buildKey('campaigns', `profit:${campaignId}`, undefined, ['org', 'campaign', 'profit']),
}
...
export const AnalyticsKeys = {
    ...
    dashboard: (orgId: string, period?: string): CacheKey =>
        buildKey('analytics', `dashboard:${orgId}`, period, ['org', 'stats', 'dashboard']),
...
export const ExternalKeys = {
    ...
    saleorProduct: (saleorId: string): CacheKey =>
        buildKey('saleor', `product:${saleorId}`, undefined, ['external', 'saleor', 'product']),
    ...
    saleorChannel: (channelId: string): CacheKey =>
        buildKey('saleor', `channel:${channelId}`, undefined, ['external', 'saleor', 'channel']),
    ...
    saleorVoucher: (voucherId: string): CacheKey =>
        buildKey('saleor', `voucher:${voucherId}`, undefined, ['external', 'saleor', 'voucher']),
}
