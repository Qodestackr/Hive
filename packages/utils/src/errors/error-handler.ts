...
import { type PromcoError, type ErrorContext, toPromcoError, ErrorCode } from "./error"
...
export class ErrorHandler {
   ...
   */
  handleApiError(error: unknown, operation: string, context?: ErrorContext): Response {
    ...
    if (promcoError.context) {
      Object.keys(promcoError.context).forEach(key => {
        ...
        if (!['operation', 'timestamp', 'traceId', 'spanId', 'url', 'method', 'params'].includes(key)) {
  ...
  private addErrorToSpan(span: Span, error: PromcoError, operation: string): void {
    span.setStatus({
  ...
  private logStructuredError(error: PromcoError, operation: string, includeStack: boolean): void {
    const logData = {
    ...
    if (error.statusCode >= 500) {
      console.error(`[PROMCO:ERROR] ${operation}:`, JSON.stringify(logData, null, 2))
    } else if (error.statusCode >= 400) {
      console.warn(`[PROMCO:WARN] ${operation}:`, JSON.stringify(logData, null, 2))
    } else {
      console.info(`[PROMCO:INFO] ${operation}:`, JSON.stringify(logData, null, 2))
    }
  }
...
export function handleApiError(error: unknown, operation: string, context?: ErrorContext): Response {
  return errorHandler.handleApiError(error, operation, context)
}

export function handleError(error: unknown, options: ErrorHandlerOptions): PromcoError {
