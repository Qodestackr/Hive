...
export const createError = {
	validation: (message: string, field?: string, context?: ErrorContext) =>
		new PromcoError(ErrorCode.VALIDATION_ERROR, message, 400, {
			...context,
			field,
		}),
	unauthorized: (
		...
		context?: ErrorContext,
	) => new PromcoError(ErrorCode.UNAUTHORIZED, message, 401, context),
	forbidden: (message: string = "Access forbidden", context?: ErrorContext) =>
		new PromcoError(ErrorCode.FORBIDDEN, message, 403, context),
	notFound: (resource: string, context?: ErrorContext) =>
		new PromcoError(ErrorCode.RECORD_NOT_FOUND, `${resource} not found`, 404, {
			...context,
			resource,
		}),
	conflict: (message: string, context?: ErrorContext) =>
		new PromcoError(ErrorCode.DUPLICATE_RECORD, message, 409, context),
	...
	rateLimit: (message: string = "Too many requests", context?: ErrorContext) =>
		new PromcoError(ErrorCode.RATE_LIMIT_EXCEEDED, message, 429, context),
	...
	internal: (
		...
		context?: ErrorContext,
	) => new PromcoError(ErrorCode.INTERNAL_SERVER_ERROR, message, 500, context),
};
...
export function toPromcoError(
	...
	if (error instanceof Error) {
		...
		if (
			...
			return createError.unauthorized(error.message, context);
		}
		...
		if (
			...
			return createError.forbidden(error.message, context);
		}
		...
		if (
			...
			return createError.validation(error.message, undefined, context);
		}
		...
		if (
			...
			return createError.external("External Service", error, context);
		}
...
export const errorLogger = {
	...
	logError: (
		...
		const promcoError =
			error instanceof PromcoError ? error : toPromcoError(error);
