...
export const sendWhatsAppNotification = async (
    ...
    try {
        ...
        throw new Error(
            `Failed to send WhatsApp notification: ${err instanceof Error ? err.message : String(err)
            }`
        );
    }
};
...
export const sendSMSNotification = async (
    ...
    try {
        ...
        throw new Error(
            `Failed to send SMS notification: ${err instanceof Error ? err.message : String(err)
            }`
        );
    }
};
...
export const sendPushNotification = async (
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => null);
            throw new Error(
                `FCM API error: ${response.status} ${response.statusText
                } ${JSON.stringify(errorData)}`
            );
        }
