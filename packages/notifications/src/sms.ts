...
export class SMSService {
    ...
    async sendSMS(options: SMSOptions): Promise<NotificationResult> {
        try {
            ...
            return {
                success: false,
                error: result.SMSMessageData?.Recipients?.[0]?.status || "SMS send failed",
            }
        } catch (error) {
    ...
    async sendOrderConfirmation(phone: string, orderNumber: string, amount: number): Promise<NotificationResult> {
    ...
    async sendPaymentConfirmation(phone: string, amount: number, reference: string): Promise<NotificationResult> {
        const message = `Payment of KES ${amount.toLocaleString()} received. Reference: ${reference}. Thank you!`
        return this.sendSMS({ to: phone, message })
    }

    async sendLowStockAlert(phone: string, productName: string, currentStock: number): Promise<NotificationResult> {
