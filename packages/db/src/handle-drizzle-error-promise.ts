...
export function handleDrizzleError(
	...
	if (pgCode === PG_ERROR_CODES.UNIQUE_VIOLATION) {
		...
		throw createError.conflict(`Duplicate entry for ${field || "field"}`, {
			...context,
			field,
			table,
			constraint,
	...
	if (pgCode === PG_ERROR_CODES.FOREIGN_KEY_VIOLATION) {
		...
		throw createError.validation(`Referenced resource does not exist`, field, {
			...context,
			table,
			constraint,
			detail,
	...
	if (pgCode === PG_ERROR_CODES.NOT_NULL_VIOLATION) {
		...
		throw createError.validation(`${field || "Field"} cannot be null`, field, {
			...context,
			table,
			constraint,
			pgCode,
	...
	if (pgCode === PG_ERROR_CODES.CHECK_VIOLATION) {
		...
		throw createError.validation(`Value violates check constraint`, field, {
			...context,
			table,
			constraint,
			detail,
	...
	if (pgCode === PG_ERROR_CODES.QUERY_CANCELED) {
		throw createError.timeout(context.operation || "database query", {
			...context,
			pgCode,
			detail: message,
		});
