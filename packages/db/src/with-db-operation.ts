...
import { withErrorTrace, SpanKind, createError, type ErrorContext } from '@repo/utils';
    ...
    return withErrorTrace(
        {
            ...
            attributes: {
                ...
                ...(config.context && Object.entries(config.context).reduce((acc, [key, value]) => {
        ...
        async (span) => {
            try {
                ...
                const { handleDrizzleError } = require('./handle-drizzle-error-traditional');
