...
import { createError, toPromcoError, type PromcoError } from '@repo/utils';
 ...
 */
export async function executeQuery<TData, TVariables extends Record<string, any> = Record<string, any>>(
 ...
 * @throws {PromcoError} Always throws PromcoError on failure
 */
export async function executeMutation<TData, TVariables extends Record<string, any> = Record<string, any>>(
    document: TypedDocumentNode<TData, TVariables> | string,
    variables: TVariables,
    context: SaleorContext
): Promise<TData> {
 ...
 * @throws {PromcoError} Always throws PromcoError on failure
 */
export async function executeSaleorRequest<TData, TVariables extends Record<string, any> = Record<string, any>>(
    document: TypedDocumentNode<TData, TVariables> | string,
    variables: TVariables,
    context: SaleorContext
): Promise<TData> {
    try {
        ...
        if (!process.env.SALEOR_API_URL || !process.env.SALEOR_APP_TOKEN) {
            throw createError.internal('Saleor client not configured. Missing SALEOR_API_URL or SALEOR_APP_TOKEN', {
 ...
 * - Unknown errors
 */
function normalizeSaleorError(error: unknown, context: SaleorContext): PromcoError {
    ...
    if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (errors.length > 0) {
            ...
            if (!firstError) {
                return createError.external('Saleor', new Error('Unknown GraphQL error'), {
    ...
    if (error && typeof error === 'object' && 'code' in error) {
        ...
        if (netError.code === 'ECONNREFUSED' || netError.code === 'ENOTFOUND') {
            return createError.external('Saleor', new Error('Saleor API unavailable'), {
                channelSlug: context.channelSlug,
                organizationId: context.organizationId,
