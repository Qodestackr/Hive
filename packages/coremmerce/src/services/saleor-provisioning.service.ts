 ...
 * 
 * Pattern matches: packages/services/src/billing.service.ts
 */

import { Effect, Schedule } from "effect"
import db, { withDrizzleErrors, saleorChannels, warehouses, eq, and } from "@repo/db"
import { GenericDatabaseError, type DatabaseError } from "@repo/utils/errors/domain"
...
export const saleorProvisioningService = {
    ...
    > {
        return Effect.gen(function* () {
            ...
            const org = yield* withDrizzleErrors(
                "organizations",
                "findFirst",
                () =>
            ...
            try {
                ...
                const warehouse = yield* createWarehouseEffect(mutations.warehouse, state)
...
function createChannelEffect(
    channelInput: { name: string; slug: string; currencyCode: string; defaultCountry: CountryCode },
...
> {
    return Effect.gen(function* () {
        ...
        const activationErrors = (activateResult as any)?.channelActivate?.errors || []
...
function createWarehouseEffect(
    warehouseInput: { name: string; slug: string; email: string; address: AddressInput },
    state: ProvisioningState
): Effect.Effect<
    { id: string; slug: string },
    SaleorWarehouseCreationFailed,
...
function createShippingZoneEffect(
    ...
    channelId: string,
    warehouseId: string
): Effect.Effect<{ id: string } | undefined, SaleorShippingZoneCreationFailed, never> {
    return Effect.tryPromise({
        try: async () => {
            ...
            console.log(
                `✅ Shipping zone created: ${shippingZone.id}`
            )
