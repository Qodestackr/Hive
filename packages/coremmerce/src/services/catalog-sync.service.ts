...
import db, { withDrizzleErrors, products as productsTable, eq } from "@repo/db"
...
export const catalogSyncService = {
    ...
    > {
        return Effect.gen(function* () {
            ...
            console.log(`📡 Target: Channel ${saleorChannel.saleorChannelId}, Warehouse ${warehouse.saleorWarehouseId}`)

            // Step 2: Fetch master catalog products
            const masterProducts = yield* getMasterCatalogProductsEffect(masterChannelSlug)

            if (masterProducts.length === 0) {
                console.warn(`⚠️ No products found in master catalog ${masterChannelSlug}`)
                return {
                    success: true,
                    totalProducts: 0,
                    successCount: 0,
            ...
            const state: SyncState = { synced: 0, failed: 0, skipped: 0, failedProducts: [] }
...
> {
    return Effect.gen(function* () {
        const warehouse = yield* withDrizzleErrors(
            "warehouses",
            "findFirst",
            () =>
...
function getMasterCatalogProductsEffect(
    ...
    return Effect.gen(function* () {
        ...
        const products = yield* withDrizzleErrors(
            "products",
            "findMany",
            () =>
...
function syncProductEffect(
    ...
    return Effect.gen(function* () {
        ...
        try {
            // Build mutation
            const mutation = buildProductSyncMutation(product, channelId, warehouseId)
