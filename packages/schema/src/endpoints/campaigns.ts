...
// Pre-flight profit check
registerRoute({
	method: "post",
	path: "/api/v1/campaigns/profit-check",
	summary: "Pre-flight profitability check",
	description:
		"Check if a discount will be profitable based on current FIFO costs. Returns recommendations if not profitable.",
...
registerRoute({
	...
	params: z.object({
		id: z
			.string()
			.openapi({ param: { name: "id", in: "path" }, example: "camp_123" }),
	}),
	response: CampaignResponseSchema,
	errors: {
...
registerRoute({
	...
	description:
		"Get real-time campaign performance stats including profit metrics.",
	tags: ["Campaigns", "Profit Intelligence"],
	params: z.object({
		id: z.string().openapi({ param: { name: "id", in: "path" } }),
	}),
