...
export const AlertSettingsUpdateSchema = z
	.object({
		autoPauseEnabled: z.boolean().optional().openapi({
			description:
				"Enable automatic campaign pause when profit margin drops below threshold",
			example: true,
		}),
		pauseThreshold: z.number().min(0).max(100).optional().openapi({
			description:
				"Profit margin percentage threshold for auto-pause (e.g., 10 = 10%)",
			example: 10.0,
		}),
...
export const AlertSettingsResponseSchema = z
	.object({
		campaignId: ID,
...
export const ProfitCheckRequestSchema = z
	.object({
		createAlertIfBelowThreshold: z.boolean().default(false).openapi({
			description:
				"Automatically create alert if profit margin is below threshold",
			example: false,
		}),
	})
	.openapi("ProfitCheckRequest");

export const ProfitCheckResponseSchema = z
	.object({
		campaignId: ID,
...
export const BulkProfitCheckRequestSchema = z
	.object({
		campaignIds: z
			.array(ID)
			.min(1)
...
export const BulkProfitCheckResponseSchema = z
	.object({
...
export const AutoPauseActionResponseSchema = z
	.object({
		campaignId: ID,
...
export type BulkProfitCheckRequest = z.infer<
	typeof BulkProfitCheckRequestSchema
>;
export type BulkProfitCheckResponse = z.infer<
	typeof BulkProfitCheckResponseSchema
>;
export type AutoPauseActionResponse = z.infer<
	typeof AutoPauseActionResponseSchema
>;
