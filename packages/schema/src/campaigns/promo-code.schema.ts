...
// ============================================================================
// PROMO CODE SCHEMAS (Single-use, tracked)
// ============================================================================
...
export const PromoCodeCreateSchema = z.object({
    ...
    code: z.string().min(1, "Promo code is required").optional().openapi({ example: 'HENNY20-ABC123' }),
    discountType: PromoCodeDiscountType,
    discountValue: z.number().nonnegative("Discount value must be non-negative").openapi({ example: 20 }),
    expiresAt: z.string().datetime(),
    metadata: z.record(z.string(), z.any()).optional(),
}).openapi('PromoCodeCreate');

export const BulkPromoCodeCreateSchema = z.object({
...
export const PromoCodeRedeemSchema = z.object({
    code: z.string().min(1, "Promo code is required").openapi({ example: 'HENNY20-ABC123' }),
    saleorOrderId: z.string().optional(),
    quantityRedeemed: z.number().int().positive().default(1),
    originalPrice: z.number().nonnegative("Original price is required").openapi({ example: 11000 }),
}).openapi('PromoCodeRedeem');

export const PromoCodeRedemptionResponseSchema = z.object({
    success: z.boolean(),
...
export const PromoCodeListQuerySchema = z.object({
    ...
    sortBy: z.enum(["code", "createdAt", "redeemedAt", "actualProfit"]).default("createdAt"),
    sortOrder: z.enum(["asc", "desc"]).default("desc"),
}).openapi('PromoCodeListQuery');

export const PromoCodeResponseSchema = PromoCodeSchema.openapi('PromoCodeResponse');
...
export const BulkPromoCodeResponseSchema = z.object({
    success: z.boolean(),
...
export type PromoCodeRedemptionResponse = z.infer<typeof PromoCodeRedemptionResponseSchema>;
