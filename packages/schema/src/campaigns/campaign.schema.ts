...
import { CampaignStatus, CampaignType, Platform } from "../shared/enums.schema.js";

// ============================================================================
// CAMPAIGN SCHEMAS (Core Promo Engine)
// ============================================================================

export const CampaignTargetSegmentSchema = z.object({
...
export const CampaignCreateSchema = z.object({
    name: z.string().min(1, "Campaign name is required").openapi({ example: 'Weekend Flash Sale' }),
    type: CampaignType,
    messageTemplate: z.string().min(1, "Message template is required").openapi({ example: 'Get 20% off Hennessy this weekend!' }),
    platforms: z.array(Platform).default(["whatsapp"]),
    ...
    offerType: z.enum(["percentage_discount", "fixed_discount", "buy_x_get_y", "free_delivery"]),
    ...
    maxRedemptions: z.number().int().positive().optional().openapi({ example: 100 }),
...
export const CampaignListQuerySchema = z.object({
    ...
    sortBy: z.enum(["name", "status", "actualProfit", "capturesCount", "createdAt"]).default("createdAt"),
    sortOrder: z.enum(["asc", "desc"]).default("desc"),
}).openapi('CampaignListQuery');

export const CampaignResponseSchema = CampaignSchema.openapi('CampaignResponse');
...
export const CampaignProfitabilityCheckSchema = z.object({
    productId: z.string().min(1, "Product ID is required").openapi({ example: 'prod_123' }),
    discountPercent: z.number().min(0).max(100, "Discount must be between 0-100%").openapi({ example: 20 }),
    discountAmount: z.number().nonnegative().optional(),
}).openapi('CampaignProfitabilityCheck');

export const CampaignProfitabilityResponseSchema = z.object({
...
export const CampaignGenerateCodesSchema = z.object({
    count: z.number().int().positive().max(1000).openapi({ example: 100 })
}).openapi('CampaignGenerateCodes');

export const CampaignGenerateCodesResponseSchema = z.object({
    success: z.boolean(),
    generated: z.number(),
    codes: z.array(z.string())
}).openapi('CampaignGenerateCodesResponse');

export const CampaignExportCodesResponseSchema = z.object({
    csv: z.string(),
    totalCodes: z.number(),
...
export type CampaignProfitabilityCheck = z.infer<typeof CampaignProfitabilityCheckSchema>;
export type CampaignProfitabilityResponse = z.infer<typeof CampaignProfitabilityResponseSchema>;
...
export type CampaignGenerateCodesResponse = z.infer<typeof CampaignGenerateCodesResponseSchema>;
export type CampaignExportCodesResponse = z.infer<typeof CampaignExportCodesResponseSchema>;

export const CampaignLtvAnalysisResponseSchema = z.object({
    campaignId: z.string(),
...
export type CampaignLtvAnalysisResponse = z.infer<typeof CampaignLtvAnalysisResponseSchema>;
