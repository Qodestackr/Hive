...
export const ProductCreateSchema = z
	.object({
		name: z
			...
			.openapi({ example: "Hennessy XO" }),
		sku: z
			.string()
			.min(1, "SKU is required")
			.openapi({ example: "HENN-XO-750" }),
		category: z.string().optional().openapi({ example: "Cognac" }),
		description: z
			.string()
			.optional()
			.openapi({ example: "Premium cognac aged 10+ years" }),
...
export const ProductUpdateSchema =
	ProductCreateSchema.partial().openapi("ProductUpdate");
...
export const ProductListQuerySchema = z
	.object({
		...
		sortBy: z
			.enum(["name", "sku", "basePrice", "currentStockQuantity", "createdAt"])
			.default("name"),
 ...
 */
export const ProductPriceUpdateSchema = z
	.object({
		basePrice: z
			.number()
			.nonnegative("Price must be non-negative")
			.openapi({ example: 11500 }),
 ...
 */
export const BulkProductPriceUpdateSchema = z
	.object({
		updates: z
			.array(
				z.object({
...
export type BulkProductPriceUpdate = z.infer<
	typeof BulkProductPriceUpdateSchema
>;
