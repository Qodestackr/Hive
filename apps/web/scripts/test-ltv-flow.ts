...
async function main() {
	...
	try {
		...
		if (redemption.netRevenue !== 4500) {
			throw new Error(
				`Expected Net Revenue 4500, got ${redemption.netRevenue}`,
			);
		}
		...
		if (customer?.attributedRevenue !== 4500) {
			throw new Error(
				`Expected Attributed Revenue 4500, got ${customer?.attributedRevenue}`,
			);
		}

		// 8. Verify Campaign LTV Analysis
		console.log("8. Verifying Campaign LTV Analysis...");
		const analysis = await campaignService.getCampaignLtvAnalysis(
			campaignId,
			orgId,
		);
		...
		console.log(
			...
			analysis.totalAttributedRevenue,
		);

		if (analysis.totalAttributedRevenue !== 4500) {
			throw new Error(
				`Expected Analysis Attributed Revenue 4500, got ${analysis.totalAttributedRevenue}`,
			);
		}
