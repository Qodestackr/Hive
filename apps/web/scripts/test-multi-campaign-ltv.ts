...
async function main() {
	...
	try {
		...
		console.log(
			`   ✅ Redeemed from Campaign A: KES ${redemptionA.netRevenue}`,
		);
		...
		const analysisA1 = await campaignService.getCampaignLtvAnalysis(
			campaignAId,
			orgId,
		);
		...
		console.log(
			`   Campaign A Attributed Revenue: ${analysisA1.totalAttributedRevenue}`,
		);

		if (analysisA1.totalCohortLTV !== 4500) {
			throw new Error(
				`❌ Campaign A LTV should be 4500, got ${analysisA1.totalCohortLTV}`,
			);
		}
		...
		// ===================================================================
		console.log(
			"7. Customer now opts into Campaign B (optInCampaignId changes)...",
		);
		await db
			.update(customers)
		...
		console.log(
			`   ✅ Redeemed from Campaign B: KES ${redemptionB.netRevenue}`,
		);
		...
		const analysisA2 = await campaignService.getCampaignLtvAnalysis(
			campaignAId,
			orgId,
		);
		...
		const analysisB = await campaignService.getCampaignLtvAnalysis(
			campaignBId,
			orgId,
		);
		...
		if (analysisB.totalCohortLTV !== 4250) {
			throw new Error(
				`❌ Campaign B LTV should be 4250, got ${analysisB.totalCohortLTV}`,
			);
		}
		...
		console.log(
			`   Customer Attributed Revenue: ${customer?.attributedRevenue}`,
		);
		...
		console.log(
			`   ✅ Customer attributedRevenue = ${expectedGlobalRevenue} (4500 + 4250, correct!)`,
		);
		...
		console.log(
			`   Customer Global Total:  KES ${customer?.attributedRevenue}`,
		);
		console.log("\n✅ Campaigns remain isolated (no cross-contamination)");
		console.log("✅ Customer attributedRevenue accumulates correctly (global)");
		console.log(
			"✅ optInCampaignId change did NOT corrupt Campaign A's LTV(historical cohort LTV)",
		);
