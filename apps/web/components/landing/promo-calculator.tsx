...
export function PromoCalculator() {
	...
	const suggestedDiscount = Math.max(
		0,
		Math.min(100, ((productPrice - unitCost * 1.1) / productPrice) * 100),
	);
						...
						<div className="flex items-start gap-4">
							<div
								className={`flex h-14 w-14 items-center justify-center rounded-2xl ${
									isProfitable ? "bg-success/10" : "bg-error/10"
								}`}
							>
								{isProfitable ? (
							...
							<div className="flex-1 space-y-4">
								<div>
									...
									>
										{isProfitable
											? "✓ This Promo is Profitable"
											: "⚠ This Promo Loses Money"}
									</h3>
								...
								<div className="space-y-2">
									...
									<p className="text-sm text-muted-foreground">
										...
										<span className="font-bold text-accent">
											{suggestedDiscount.toFixed(0)}% discount
										</span>{" "}
										to maintain a healthy 10% profit margin. This would give you{" "}
										<span className="font-bold text-success">
											{formatKES(
												(productPrice * (1 - suggestedDiscount / 100) -
													unitCost) *
													100,
											)}
										</span>{" "}
										profit on 100 redemptions.
									</p>
								</div>
							</div>
						</motion.div>
						...
						>
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<p className="text-sm text-muted-foreground mt-1">
										This promotion maintains healthy margins. With Promco, you'd
										get real-time alerts if profit starts dropping mid-campaign.
									</p>
								</div>
							</div>
						</motion.div>
