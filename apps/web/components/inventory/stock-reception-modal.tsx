...
import { PackagePlus, ScanBarcode, Calculator, ArrowRight, CheckCircle2, TrendingUp, Truck } from "lucide-react"
...
const PRODUCT_DB = [
    { id: "1", name: "Four Cousins Sweet Red", currentCost: 7800, stock: 45, image: "/wine-still-life.png" },
    { id: "2", name: "Jameson Irish Whiskey", currentCost: 2300, stock: 12, image: "/amber-whiskey-glass.png" },
]
...
export function StockReceptionModal() {
    ...
    const [selectedProduct, setSelectedProduct] = useState<(typeof PRODUCT_DB)[0] | null>(null)
    ...
    const enteredCost = Number.parseInt(newCost) || 0
            ...
            <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden bg-zinc-950 border-zinc-800 text-zinc-100 gap-0">
                ...
                <div className="p-6 bg-zinc-900/50 border-b border-zinc-800">
                    <div className="flex items-center justify-between mb-2">
                        <h2 className="text-xl font-semibold tracking-tight">New Stock Arrival</h2>
                            ...
                            >
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 mt-4">
                                        <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Recent Suggestions</p>
                                        {PRODUCT_DB.map((product) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <h4 className="font-medium text-zinc-200">{product.name}</h4>
                                                    <p className="text-xs text-zinc-500">Current Stock: {product.stock} units</p>
                                                </div>
                                                {selectedProduct?.id === product.id && <CheckCircle2 className="w-5 h-5 text-indigo-500" />}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {step === 2 && selectedProduct && (
                            ...
                            >
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-zinc-400">Buying Price (Total)</Label>
                                        <div className="relative">
                                            <span className="absolute left-3 top-3 text-zinc-500 font-mono">KES</span>
                                            <Input
                                                type="number"
                                                value={newCost}
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            <div
                                                className={cn(
                                                    "p-2 rounded-lg mt-1",
                                                    isPriceHigher ? "bg-amber-500/10 text-amber-500" : "bg-emerald-500/10 text-emerald-500",
                                                )}
                                            >
                                                {isPriceHigher ? <TrendingUp className="w-4 h-4" /> : <Calculator className="w-4 h-4" />}
                                            </div>
                                            <div>
                                                <h4 className="text-sm font-medium text-zinc-200">
                                                    {isPriceHigher ? "Price Hike Detected" : "Cost Analysis"}
                                                </h4>
                                                <p className="text-sm text-zinc-400 mt-1">
                                                    Previous unit cost was{" "}
                                                    <span className="text-zinc-300 font-mono">KES {oldCost.toLocaleString()}</span>. This batch
                                                    costs{" "}
                                                    <span
                                                        className={cn("font-mono font-bold", isPriceHigher ? "text-amber-500" : "text-zinc-300")}
                                                    >
                                                        KES {(enteredCost / Number.parseInt(quantity)).toLocaleString()}
                                                    </span>
                                                    .
                                                </p>
                                                {isPriceHigher && (
                                                    <div className="mt-3 text-xs bg-amber-500/10 text-amber-400 p-2 rounded border border-amber-500/20">
                                                        <strong>Insight:</strong> Your existing {selectedProduct.stock} bottles were bought cheaper.
                                                        We'll track this so you know exactly which ones make you more profit!
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    >
                                        ...
                                        >
                                            {landedCostLater && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                                        </div>
                        ...
                        {step === 3 && selectedProduct && (
                            ...
                            >
                                <div className="text-center space-y-2 py-4">
                                    ...
                                    <h3 className="text-xl font-semibold text-zinc-100">Stock Recorded!</h3>
                                    <p className="text-zinc-400 max-w-[280px] mx-auto">
                                        Added <span className="text-white font-medium">{quantity} units</span> of {selectedProduct.name} to
                                        your inventory.
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-zinc-900 rounded-xl border border-zinc-800">
                                        <div className="text-xs text-zinc-500 mb-1">New Total Stock</div>
                                        <div className="text-xl font-mono text-zinc-200">
                                            {selectedProduct.stock + Number.parseInt(quantity)} units
                                        </div>
                                    </div>
                                    <div className="p-3 bg-zinc-900 rounded-xl border border-zinc-800">
                                        <div className="text-xs text-zinc-500 mb-1">Batch Value</div>
                                        <div className="text-xl font-mono text-zinc-200">
                                            KES {Number.parseInt(newCost).toLocaleString()}
                                        </div>
                                    </div>
                                </div>
                                ...
                                {landedCostLater && (
                                    <div className="flex gap-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                                        ...
                                        <div className="text-xs text-blue-300">
                                            <strong>Pending Action:</strong> We've marked this PO as "Pending Landed Costs". When the
                                            transporter bill arrives, just tap the notification to update the final cost.
                                        </div>
                                    </div>
                                )}

                                <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white h-12" onClick={reset}>
                                    Done
                                </Button>
                            </motion.div>
                        )}
