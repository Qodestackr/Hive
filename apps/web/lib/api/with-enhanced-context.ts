...
import { withWorkspaceContext, type WorkspaceHandler, type WithWorkspaceOptions } from "./with-workspace-context"
import { withApiTrace } from "@repo/utils"
...
export function withEnhancedApiContext(
  ...
  const workspaceHandler = withWorkspaceContext(handler as WorkspaceHandler, options)
  const tracedHandler = withApiTrace(options.operationName, workspaceHandler, options.errorContext)
 ...
 */
function withSimpleApiContext(handler: SimpleApiHandler, options: EnhancedApiOptions) {
