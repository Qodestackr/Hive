...
export interface WithWorkspaceOptions {
	...
	requiredBusinessTypes?: Array<
		"retailer" | "wholesaler" | "distributor" | "brand_owner"
	>;
 ...
 */
async function fetchOrganizationExtras(organizationId: string): Promise<{
...
async function validateMembership(
	...
	if (!currentMember) {
		...
		const pendingInvite = await checkPendingInvitation(
			userEmail,
			organizationId,
		);
	...
	return async (
		...
		return withErrorTrace(
			...
			async (span) => {
				...
				const context: WorkspaceContext = extractContextIds(
					params,
					searchParams,
				);
				...
				if (!session?.user?.id) {
					...
					if (process.env.NODE_ENV === "production" && !session) {
						console.warn(
							"Build time: No session available, returning empty response",
						);
						return NextResponse.json(
							{ error: "Build time - no session" },
							{ status: 200 },
						);
					}
				...
				const { organizationId, organizationSlug } = resolveOrganizationId(
					params,
					searchParams,
				);
				...
				const currentMember = authMembers?.find(
				...
				if (
					...
					for (const perm of options.requiredPermissions) {
						...
						if (!resource || !action) {
							throw new Error(
								`Invalid permission format: ${perm}. Use "resource.action"`,
							);
						}
